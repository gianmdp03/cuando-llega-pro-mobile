...
function arrayFrom(
  raw: string,
  property: 'lineas' | 'calles' | 'paradas'
...
export function parseMgpLines(raw: string): TransitLineDTO[] {
  return arrayFrom(raw, 'lineas').map((item) => ({
    id: text(item.CodigoLineaParada),
    codigo: text(item.Descripcion),
    descripcion: text(item.Descripcion),
    codigoEntidad: text(item.CodigoEntidad) || null,
    codigoEmpresa: numberOrNull(item.CodigoEmpresa),
  }));
}
export function parseMgpStreets(raw: string): TransitStreetDTO[] {
  return arrayFrom(raw, 'calles')
    .map((item) => ({
      codigo: text(item.Codigo),
      descripcion: stripCitySuffix(text(item.Descripcion)),
    }))
...
export function parseMgpStopsWithFlag(raw: string): TransitStopWithFlagDTO[] {
  return arrayFrom(raw, 'paradas').map((item) => ({
    codigo: text(item.Codigo),
    identificador: text(item.Identificador),
    descripcion: text(item.Descripcion),
    abreviaturaBandera: text(item.AbreviaturaBandera),
    abreviaturaAmpliadaBandera: text(item.AbreviaturaAmpliadaBandera),
    latitudParada: numberOrNull(item.LatitudParada),
    longitudParada: numberOrNull(item.LongitudParada),
  }));
}
